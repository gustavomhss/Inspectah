...
SCORECARD="$REPO_ROOT/out/scorecards/S6_G2_field_designer.json"
EVIDENCE_DIR="$REPO_ROOT/out/evidence/S6_G2_field_designer"
mkdir -p "$(dirname "$SCORECARD")" "$EVIDENCE_DIR"

OUTPUT_JSON="$EVIDENCE_DIR/fields_preview.json"
...
json.dump({
    ...
    "status": status,
    "details": {
        "sources_total": len(data.get('samples', {})),
