...
if [[ "$BRANCH" != "s12_ingestao_continua_comunidade_v0" && "$BRANCH" != "main" ]]; then
  status="FAIL"
  notes+=("Branch inválida para S12: $BRANCH")
fi
...
python3 - <<'PY' "$ENV_SNAPSHOT" "$BRANCH" "$ORIGIN" "${REQUIRED_DOCS[@]}" "${missing_docs[@]}"
import json
import sys
...
branch = sys.argv[2]
origin = sys.argv[3]
required_docs = sys.argv[4:8]
missing_docs = sys.argv[8:]
payload = {
    "branch": branch,
    "origin": origin,
