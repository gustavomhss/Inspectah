...
EVIDENCE_DIR="$ROOT_DIR/out/evidence/S26_G0_scope_and_baseline"
SCORECARD_PATH="$SCORECARD_DIR/S26_G0_scope_and_baseline.json"
LOG_PATH="$EVIDENCE_DIR/g0_scope_and_baseline.log"

mkdir -p "$SCORECARD_DIR" "$EVIDENCE_DIR"

EXPECTED_DOCS=(
  "$ROOT_DIR/docs/sprint_26_cap_1_contexto.md"
  "$ROOT_DIR/docs/sprint_26_cap_2_gates_e_metricas.md"
  "$ROOT_DIR/docs/sprint_26_cap_3_arquitetura_e_filemap.md"
  "$ROOT_DIR/docs/sprint_26_cap_4_execucao_e_evidencias.md"
)
...
{
  echo "[S26_G0] docs_present=$docs_present"
  echo "[S26_G0] frontend_deps_ok=$frontend_deps_ok"
  echo "[S26_G0] backend_deps_ok=$backend_deps_ok"
  echo "[S26_G0] design_system_skeleton_present=$design_system_skeleton_present"
  echo "[S26_G0] sources_console_skeleton_present=$sources_console_skeleton_present"
  if [[ ${#docs_missing[@]} -gt 0 ]]; then
    echo "[S26_G0] Docs faltantes:"
    printf '%s\n' "${docs_missing[@]}"
  fi
  if [[ ${#dirs_missing[@]} -gt 0 ]]; then
    echo "[S26_G0] Diretórios faltantes:"
    printf '%s\n' "${dirs_missing[@]}"
  fi
} | tee "$LOG_PATH"
...
scorecard = {
    "gate": "S26_G0_scope_and_baseline",
    "timestamp": timestamp,
    "status": status,
    "metrics": {
...
print(f"[S26_G0] Scorecard salvo em {scorecard_path}")
PY
