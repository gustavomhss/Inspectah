...
EVIDENCE_DIR="$ROOT_DIR/out/evidence/S10_G5"
SCORECARD="$SCORECARD_DIR/S10_G5_e2e_domain_A.json"
LOG_FILE="$EVIDENCE_DIR/tests.log"
REPORT_FILE="$EVIDENCE_DIR/domain_a_report.json"
...
python3 "$ROOT_DIR/bin/s5_pytest_shim.py" \
  "$ROOT_DIR/tests/pipelines/test_s10_domain_a_obras.py" \
  >"$LOG_FILE" 2>&1
shim_exit=$?
set -e
...
import json
import sys
from inspectah.pipelines import s10_domain_a_obras as pipeline
...
cat >"$SCORECARD" <<JSON
{
  ...
  "checks": [
    {
      "id": "domain-a-tests",
      "description": "Rodar tests/pipelines/test_s10_domain_a_obras.py via shim",
      "status": "$test_status",
