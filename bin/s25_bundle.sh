...
STAGING_DIR="${ROOT_DIR}/out/S25_bundle"
MANIFEST_PATH="${STAGING_DIR}/bundle_manifest.json"
BUNDLE_ZIP="${BUNDLES_DIR}/inspectah_s25_evidence_bundle.zip"
BUNDLE_LOG="${EVIDENCE_DIR}/S25_bundle/bundle.log"

mkdir -p "${SCORECARDS_DIR}" "${EVIDENCE_DIR}/S25_bundle" "${BUNDLES_DIR}"
rm -rf "${STAGING_DIR}"
mkdir -p "${STAGING_DIR}/scorecards" "${STAGING_DIR}/evidence"

set +e
scorecard_files=($(find "${SCORECARDS_DIR}" -maxdepth 1 -type f -name "S25_*.json"))
set -e
if [ ${#scorecard_files[@]} -gt 0 ]; then
  cp "${scorecard_files[@]}" "${STAGING_DIR}/scorecards/" 2>/dev/null || true
fi

if compgen -G "${EVIDENCE_DIR}/S25*" >/dev/null; then
  cp -R ${EVIDENCE_DIR}/S25* "${STAGING_DIR}/evidence/" 2>/dev/null || true
fi
...
cat > "${BUNDLE_LOG}" <<LOG
S25 bundle generated at $(date -u +"%Y-%m-%dT%H:%M:%SZ")
Bundle: ${BUNDLE_ZIP}
