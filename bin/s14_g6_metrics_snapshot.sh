...
EVIDENCE_DIR="$ROOT_DIR/out/evidence/S14_G6"
SCORECARD_PATH="$SCORECARD_DIR/S14_G6_metrics_snapshot.json"
...
if [[ ! -f "$REPORT_PATH" ]]; then
  >&2 echo "[S14_G6] Report não encontrado em $REPORT_PATH"
  exit 1
fi
...
if status == "FAIL":
    raise SystemExit("S14_G6 falhou; consulte scorecard.")
PY

echo "[S14_G6] Status registrado em $SCORECARD_PATH"
