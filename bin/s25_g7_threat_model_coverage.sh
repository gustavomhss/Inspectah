...
EVIDENCE_DIR="$ROOT_DIR/out/evidence/S25_G7_threat_model_coverage"
SCORECARD_PATH="$SCORECARD_DIR/S25_G7_threat_model_coverage.json"
...
echo "[S25_G7] Validando thresholds..." | tee "$THRESHOLD_LOG"
...
echo "[S25_G7] Rodando cenários adversariais..." | tee "$TEST_LOG"
set +e
...
{
  "gate_id": "S25_G7",
  "gate_name": "threat_model_coverage",
  "sprint": "S25",
  "status": "$status",
  "timestamp": "$ts",
  ...
  "human_code_score": {
    "applied": true,
    "score": 0.6,
    "notes": "Cenários adversariais básicos rodados com golden sets."
  },
  "risks": [],
...
if [[ "$status" != "GO" ]]; then
  echo "[S25_G7] NO_GO - confira $SCORECARD_PATH"
  exit 1
fi

echo "[S25_G7] GO - scorecard em $SCORECARD_PATH"
