...
EVIDENCE_DIR="$ROOT_DIR/out/evidence/S9_T3_property_and_edge_cases"
SCORECARDS_DIR="$ROOT_DIR/out/scorecards"
SUMMARY_FILE="$EVIDENCE_DIR/summary.json"
SCORECARD_FILE="$SCORECARDS_DIR/S9_T3_property_and_edge_cases.json"
...
if PYTHONPATH=. .venv/bin/python -m pytest tests/s9_t3_property -q >"$PYTEST_LOG" 2>&1; then
  TEST_STATUS="PASS"
else
  STATUS="FAIL"
  TEST_STATUS="FAIL"
...
python3 - "$SUMMARY_FILE" "$SCORECARD_FILE" "$PYTEST_LOG" "$STATUS" "$TIMESTAMP" <<'PY2'
import json
import re
...
summary = {
    "gate": "S9_T3_property_and_edge_cases",
    "status": status,
    "timestamp": timestamp,
...
echo "S9_T3_property_and_edge_cases PASS. Evidencias em $EVIDENCE_DIR"
