...
## Bloco 1.4 — Saídas Esperadas da S26 (Entregáveis & Estado Desejado)

### 1. Estado desejado ao final da Sprint

Ao final da S26, queremos poder afirmar, sem asteriscos, que:

1. **Existe um Design System Inspectah Admin v1 real, em produção interna**, com:
   - tokens definidos (paleta de cores, tipografia, scale de espaçamentos, raios de borda, sombras, estados de foco/hover/disabled/erro/sucesso);
   - um conjunto mínimo de componentes reutilizáveis para consoles admin (layout de página, shell de navegação com sidebar/header, tabela padrão, formulário padrão, botões, inputs, selects, modais, toasts, banners de estado, badges/tags);
   - documentação inicial suficiente para que qualquer dev do time saiba “como fazer uma tela nova de admin do jeito certo”.

2. **O Console de Fontes opera 100% apoiado nesse design system**, cobrindo o ciclo de vida básico de fontes:
   - visualizar a lista de fontes com filtros básicos úteis;
   - criar uma nova fonte preenchendo os campos obrigatórios com ajuda contextual clara;
   - editar uma fonte existente com segurança, entendendo impacto dos campos críticos;
   - ativar, desativar e, quando aplicável, arquivar fontes com confirmações explícitas e mensagens compreensíveis;
   - perceber, pela UI, quando uma ação foi bem-sucedida ou falhou, com mensagens e estados visuais consistentes.

3. **A experiência do operador de fontes é treinável e documentável**:
   - a partir das telas resultantes, é possível escrever um runbook simples e honesto de operação de fontes;
   - um operador técnico, não envolvido na implementação, consegue ser treinado em pouco tempo (onboarding dirigido) para realizar as operações básicas apenas via UI.

4. **O que foi feito em S26 reduz a entropia das próximas sprints**, em vez de aumentar:
   - o design system v1 é claramente extensível para os consoles de Ingestão, Debunker, Truth Console, Evidence Vault e Case Cockpit;
   - o Console de Fontes v2 não introduz atalhos ou hacks que atrapalhem S27–S32; ao contrário, ele é um exemplo a ser seguido.

### 2. Entregáveis concretos de produto

Do ponto de vista de artefatos de produto, a S26 deve sair com:

1. **Biblioteca inicial do Design System Admin v1** no frontend, contendo:
   - estrutura de pastas clara (ex.: `ui/admin/*` ou equivalente) separando componentes de base de componentes de domínio;
   - implementação dos componentes nucleares listados neste capítulo, consumidos pelo Console de Fontes;
   - definição explícita dos tokens de design (em código) e, idealmente, um ponto único de verdade para eles.

2. **Console de Fontes v2**:
   - telas de lista, criação e edição de fontes refatoradas para usar apenas componentes do design system;
   - fluxos de ativar/desativar/arquivar fontes com diálogos de confirmação, mensagens de feedback e estados visuais consistentes;
   - ajustes necessários no backend (se houver) alinhados e cobertos em Cap.3/Cap.4.

3. **Documentação mínima de uso do design system e do console**:
   - um doc curto (pode ser anexo da sprint) explicando “como criar telas admin novas usando o Design System Admin v1”;
   - um esboço de runbook de operação de fontes baseado no novo console (mesmo que seja v0, já útil para treinamento interno).

### 3. Entregáveis de engenharia e qualidade

Além dos artefatos de produto, S26 precisa produzir evidências técnicas claras de que o que foi feito é sólido:

1. **Testes automatizados ao redor do design system e do Console de Fontes**:
   - smoke tests ou unit tests pelo menos para os componentes de UI mais críticos (botões, formulários, tabela, layout);
   - testes básicos de fluxo para as operações principais do Console de Fontes (ex.: criação bem-sucedida, validação de campos obrigatórios, ativar/desativar).

2. **Integração completa com a pipeline de CI existente**:
   - o design system e o novo console passam por lint, testes e build sem exigir passos manuais extras;
   - nenhuma regressão introduzida em partes estáveis do sistema.

3. **Evidências para ORR da sprint** (detalhadas no Cap.2):
   - prints, gravações ou scripts que mostrem os fluxos principais funcionando em ambiente local/CI;
   - scorecards e bundles que registrem o estado de S26 de forma auditável.

### 4. Limites explícitos das saídas de S26

Para proteger o foco da sprint, também é importante dizer o que **não** será considerado falha de S26:

- O design system não precisa, ainda, cobrir todos os componentes imagináveis; ele precisa cobrir, com qualidade, o conjunto necessário para o Console de Fontes e para servir de base para S27–S32.
- O Console de Fontes não precisa expor, nesta sprint, métricas avançadas, healthscore completo ou dashboards sofisticados; ele precisa ser confiável e coerente para o ciclo de vida básico das fontes.
- A documentação produzida pode ser v1, desde que verdadeira, utilizável e evolutiva — não estamos buscando um “manual definitivo”, mas sim material suficiente para que outro time/squad consiga continuar a partir daqui com segurança.

Em resumo, as saídas esperadas da S26 são: **um design system admin vivo, um Console de Fontes reconstruído sobre ele e um conjunto mínimo de documentação e testes** que transformam essa combinação em base sólida para o restante do Programa 1.